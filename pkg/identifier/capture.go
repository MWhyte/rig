...
func CaptureMonoSamples(ctx context.Context, streamURL string, duration time.Duration) (samples []float64, sampleRate int, err error) {
	...
	// which makes go-mp3 fail with errors like "is_pos was too big". Scan
	// forward to the first valid MP3 frame header before decoding.
	synced, err := syncToMP3Frame(resp.Body)
	if err != nil {
...
// syncToMP3Frame scans bytes from r until it finds an MP3 frame header that
// passes isValidMP3Header, then returns a reader positioned at that byte.
// Bytes are inspected via Peek so they stay buffered for the decoder.
func syncToMP3Frame(r io.Reader) (*bufio.Reader, error) {
